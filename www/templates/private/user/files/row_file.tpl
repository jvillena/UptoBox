	...
	<div class="alert-message block-message warning">
        ...
        <div class="alert-actions">
          <a id="bnew" onclick="createFolder('{translate}tx_options_create_new_folder{/translate}');" href="#" class="btn small">
          	<img style="width:20px;vertical-align:bottom" src="{$BASE_THEMES_URL}images/icons/icon_button_folder.png"/>{translate}tx_options_new_folder{/translate}
          </a> 
          
          <a href="#" class="btn small">
          	<img style="width:20px;vertical-align:bottom" src="{$BASE_THEMES_URL}images/icons/icon_button_upload.png"/>{translate}tx_options_upload_file{/translate}
          </a>
        </div>
      </div>
 {else}
	{foreach name="files_tree" from=$aFile item=item key=key}
				<div style="border-bottom: 1px solid  #DDDDDD;" id="more_options_row_{$item.id_archivo}" class="row_f">
						<div style="clear:both;">
							  <div id="sprite_file" style="float:left;width:42px;margin-bottom: 5px;margin-top: 5px;">
								{if $item.tipo==0}
									 <img style="vertical-align:bottom" src="{$BASE_THEMES_URL}images/icons/icon_folder.png"/>
								 {else}
								 		{if $item.id_archivo_padre == 0}
								 			{Tools::getImageFromFile( {$id_usuario}, {$RUTA_WEB_ABSOLUTA} , {$BASE_THEMES_URL}, {$item.name_file}, {$item.ext}, {$item.type_file}, 0, 35,'',{$item.id_archivo})}
								 			<script>
								 				{literal}
								 				$(document).ready(function() {
														...
														innerHtml: '{/literal}{Tools::getImageFromFile( {$id_usuario}, {$RUTA_WEB_ABSOLUTA}, {$BASE_THEMES_URL}, {$item.name_file}, {$item.ext}, {$item.type_file}, 0, 80, {$item.id_archivo} )}{literal}',
														...
														themeName: 	'azure',
														themePath: 	'{/literal}{$RUTA_WEB_ABSOLUTA}{literal}libs/jquery/bubblePopup/Examples/images/jquerybubblepopup-theme',
								 		...
								 		{else}
								 			{Tools::getImageFromFile( {$id_usuario}, {$RUTA_WEB_ABSOLUTA}, {$BASE_THEMES_URL}, {$item.name_file}, {$item.ext}, {$item.type_file}, {$item.id_archivo_padre}, 30,'' ,{$item.id_archivo})}
								 			<script>
								 				{literal}
								 				$(document).ready(function() {
									 					$('#div_{/literal}{$item.id_archivo}{literal}').CreateBubblePopup();
														...
														innerHtml: '{/literal}{Tools::getImageFromFile( {$id_usuario}, {$RUTA_WEB_ABSOLUTA}, {$BASE_THEMES_URL}, {$item.name_file}, {$item.ext}, {$item.type_file}, {$item.id_archivo_padre}, 80,'' ,{$item.id_archivo})}{literal}',
