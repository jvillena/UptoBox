	...
	class RestServer {
		...
		public function  __destruct() {
			if ($this->mode == 'production' && !$this->cached) {
				if (function_exists('apc_store')) {
					...
					file_put_contents(dirname(__FILE__)."/".$this->cacheDir . '/urlMap.cache', serialize($this->map));
				}
			}
		}
		...
		protected function loadCache() {
			...
			if ($this->mode == 'production') {
				...
				elseif (file_exists(dirname(__FILE__)."/".$this->cacheDir . '/urlMap.cache')) {
					$map = unserialize(file_get_contents(dirname(__FILE__)."/".$this->cacheDir . '/urlMap.cache'));
				}
				if ($map && is_array($map)) {
					$this->map = $map;
					$this->cached = true;
				...
				if (function_exists('apc_delete')) {
					...
					@unlink(dirname(__FILE__)."/".$this->cacheDir . '/urlMap.cache');
				}
			}
		}
